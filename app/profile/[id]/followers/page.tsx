...
export default function FollowersPage() {
  ...
  const { data, isLoading } = useSWR(`/api/users/${id}/followers`, fetcher)
  const { data: userData } = useSWR(`/api/users/${id}`, fetcher)
  const followers: any[] = (data as any)?.data ?? []
  ...
  async function toggleFollow(userId: string) {
    ...
    try {
      ...
      setFollowing(prev => {
        ...
        res.is_following ? n.add(userId) : n.delete(userId)
        return n
      })
    ...
    <div className="max-w-lg mx-auto">
      ...
      {isLoading ? (
        ...
        <div className="py-16 text-center">
          <p className="text-text-secondary text-sm">No followers yet</p>
        </div>
      ) : (
            ...
            <div key={user.id} className="flex items-center gap-3 px-4 py-3">
              ...
              {me?.id !== user.id && isLoggedIn && (
                <button onClick={() => toggleFollow(user.id)}
                  className="text-xs font-semibold px-3 py-1.5 rounded-full border border-border text-text-secondary hover:border-primary hover:text-primary transition-all">
      ...
      <div className="lg:hidden">
        <div className="sticky top-0 z-50 bg-bg/90 backdrop-blur-xl border-b border-border safe-top">
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            <h1 className="font-bold">{user?.display_name || 'User'}'s Followers</h1>
          </div>
        </div>
